...
export default function FetchQuestions({ questions }: { questions: Question[] }) {
  ...
  // ✅ Wait for hydration before running login check
  useEffect(() => {
    if (!hydrated) return; // wait for Zustand to hydrate
    if (!isLoggedIn) {
      toast.error('You are not logged in, Lil bro 😎');
      const timeout = setTimeout(() => router.replace('/auth/login'), 2000); // wait 3s
  ...
  // ⏳ Wait for hydration and user before rendering
      ...
      {questions.map((q) => (
        <div key={q.$id} className="flex items-start space-x-4 mt-5">
          ...
          <div className="text-center text-white w-12">
            ...
            <hr className="mt-6 w-17" />
          </div>

          {/* Question Content */}
          <div className="flex-1">
            <Link href={`/questions/${q.$id}`} className="text-white text-xl font-semibold no-underline">
              {q.title}
            </Link>
